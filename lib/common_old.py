...
class AgentDDPG():
    ...
    """
    def __init__(self, net, device="cpu", **kwargs):
        self.net = net
        self.device = device
    ...
    def __call__(self, states, agent_states):
        ...
        actions_t = self.net(states_t) # actions tensor
        actions = actions_t.data.cpu().numpy()
        ...
        if self.ou_epsilon > 0:
            ...
            for a_state, action in zip(agent_states, actions):
                ...
                a_state += self.ou_teta * (self.ou_mu - a_state) \
                    + self.ou_sigma * np.random.normal(size=action.shape) # random noice
...
class ExperienceReplayBuffer: 
    """ !!! for HER take the last but one state because the last one will yield collision"""

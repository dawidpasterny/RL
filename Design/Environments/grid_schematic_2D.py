...
STEPS = 6 # 6 discrete steps in every direction
N = 24
...
class GridSchematic2D(Env):
    ...
    def step(self, a):
        """Termination if:
            - you walk into yourself (but not vertically)
        ...
        try:
            if a%4==0: # going left
                ...
                # it doesn't consitute a gear stage and diminished reward but it's possible
                new_col=col-step
                if new_col>=0 and self.no_go[row,new_col]!=1: 
                    # if we don't go into a wall, obstacle or itself
                    p -= step
                    self.no_go[row, new_col:col] = 1
                else: 
                    done = True
                    info = "Termination, walked into a wall/obstacle/itself"
                    reward = -1 + col/self.ncol
            elif a%4==2: # going right
                new_col = col+step
                if new_col<self.ncol and self.no_go[row,new_col]!=1: 
                        # if we don't go into a wall, obstacle or itself
                    p += step
                    self.no_go[row,col+1:new_col+1] = 1
                else:
                    done = True
                    info = "Termination, walked into a wall/obstacle/itself"
                    reward = -1 + col/self.ncol
                ...
                if a_last%4==0 or a_last%4==2: # no gear created in previous move
                    ...
                    if row < step or row+step > self.nrow or self.no_go[row-step,col]==1 or self.no_go[row+step,col]==1: 
                        done = True
                        info = "Termination, chosen gear is too large"
                        reward = -1 + col/self.ncol
                    self.no_go[row-step:row+step+1,col] = 1
                else: # creating a gear stage
                    # will transition to new row only if a valid gear state has been created
                    ...
                    if a%4==1 and a_last%4==1: # up-up, traditional gear stage
                        ...
                        if new_row-step < 0 or self.no_go[new_row-step, col]==1: 
                            done = True 
                            info = "Termination, gear stage doesn't fit into the available space"
                            reward = -1 + col/self.ncol
                        else:
                        ...
                        new_row = row+last_step+step+1
                        if new_row+step > self.nrow or self.no_go[new_row+step,col]==1:
                            done = True 
                            info = "Termination, gear stage doesn't fit into the available space"
                            reward = -1 + col/self.ncol
                        else:
                            ...
                            i *= -Fraction(last_step,step)
                            self.no_go[new_row-step:new_row+step+1,col] = 1
                    else: # down-up or up-down, planetary gear stage
                        if row < last_step+1 or row+last_step+1 > self.nrow or self.no_go[row-last_step-1,col]==1 or self.no_go[row+last_step+1,col]==1: 
                            done = True
                            info = "Termination, colision with the ring gear"
                            reward = -1 + col/self.ncol
                        else:
                            # planetary gear stage has thickness
                            if all(self.no_go[row,col-2:col+3]==1) or col==self.ncol-1 or col==0:
                                done=True
                                info = "Termination, planetary gearset is to wide to fit into the available space"
                                reward = -1 + col/self.ncol
                            elif self.no_go[row,col+1]==1: # we came from the right
                                p-=1
                                self.no_go[row-last_step-1:row+last_step+2,col-1:col+1] = 1
                            elif self.no_go[row,col-1]==1:
                                p+=1
                                self.no_go[row-last_step-1:row+last_step+2,col:col+2] = 1
                                # self.no_go[row-last_step:row+last_step+1,col+1] = 0
                                # self.no_go[row-last_step+step:row+last_step-step+1,col+1] = 1
                            try:
                                i *= Fraction(2*last_step-step,last_step-step)
                            except ZeroDivisionError:
                                done=True
                                info = "Termination, ring and planete gear cannot have same radius"
                                reward = -1 + col/self.ncol
        except IndexError:
            done=True
            info="Out of bounds"
    ...
    def reset(self, random=False):
        ...
        self.no_go = self.map.copy() # a map of visited states and obstackles
        self.ax.clear()
        return self.s
...
class ScreenOutput(ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        o = self.no_go.copy()
        o[p//self.ncol, p%self.ncol]= 0.587

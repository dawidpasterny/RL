...
def generate_random_map(rng, w, h, boundary=0.2, target=True):
    ...
    if rng.rand()<boundary:
        ...
        # Pick points not too close to the boundary
        x_s, x_t = x1 + 1.5*min(DIAMS) + rng.rand(2)*(x2-x1-3*min(DIAMS))
        y_s, y_t = y1 + 1.5*min(DIAMS) + rng.rand(2)*(y2-y1-3*min(DIAMS))
    else:
        x_s, x_t = 0.05*w + rng.rand(2)*0.9*w
        y_s, y_t = 0.05*h + rng.rand(2)*0.9*h
    m["p_start"] = [x_s,y_s]

    if target: # creates unreachable state
        m["p_target"] = [x_t,y_t]
        # A sort of random walk to generate target
        r = rng.choice(DIAMS)
        for _ in range(rng.randint(2,5)):
            r = rng.choice(DIAMS)/r
        m["i_target"]= r
...
DIAMS = np.linspace(.1,.25, 4)
I_MAX = max(DIAMS)/min(DIAMS)
ANGLES = np.arange(0, 2*np.pi, np.pi/3)
MAX_TRAJ_LEN = 8 # max length of a chain of gears
MAX_STEPS = 8 # for selfplay, when collision doesn't cause termination


class StageCreator(GoalEnv):
    def __init__(self, N, ae=None, env_map=None, boundary=0.3, seed=None, mode="goal"):
        """ Goal based environment, contained to the design of as single gearbox stage (2D task) i.e.
            ...
            achieving desired ratio. Both the gear diameters and relative angles of placement 
            are discrete!
        ...
        if ae == None:
            obs = spaces.Dict({
                    'screen': spaces.Box(low=0.0, high=1.0, shape=(1,N,N), dtype=float), 
                    'xyi': spaces.Box(low=np.array([0,0,-I_MAX]), high=np.array([1,1,I_MAX])),
                    'last_diam': spaces.Discrete(len(DIAMS))})
        else:
            out = 4 + ae.get_bottleneck_size(N)[-1] # x, y, i ,diam + hidden dim
            obs = spaces.Box(low=-1.0, high=1.0, shape=(out,))
        
        self.observation_space = spaces.Dict({
                    'observation': obs, 
                    'desired_goal': spaces.Box(low=np.array([0,0,-I_MAX]), 
                                                high=np.array([1,1,I_MAX])), # x_target, y_target, i_target
                    'achieved_goal': spaces.Box(low=np.array([0,0,-I_MAX]), 
                                                high=np.array([1,1,I_MAX]))}) # x_current, y_current, i_current

        # action is a tuple (diam, phi)
        self.action_space = spaces.Discrete(len(DIAMS)*len(ANGLES))
    ...
    def step(self, a:tuple):
        """ Irespectful of the mode returns a tuple: (obs, done, reward, info)
            - a is a tuple of (d, phi) or (d, phi, agent_idx) or 
            (d, phi, stop, agent_idx) where agent_idx is necessary to 
            distinguish who is playing in the selfplay mode (1=Alice, 0=Bob).
            In selfplay mode there is no termination upon collision, instead 
            just no change in state. Also, the rewards are calculated externally 
            so they don't matter in that case.
        """
        ...
        # Decode the actions from joint prob. action_idx
        # phi = ANGLES[a[0]//len(DIAMS)]
        # next_state[3] = d = DIAMS[a[0]%len(DIAMS)]
        phi = a[1]
        next_state[3] = d = a[0]
        ...
        # a gear stage (every step modifies d though)        
        if self.traj!=[]:
            next_state[:2] += pol2car((old_d+d)/2, phi) # new position
            next_state[2] *= -d/old_d # new ratio
        if self.check_collisions(*next_state[[0,1,3]]):
            if self.mode=="goal":
                done = True  
                # update state nevertheless to see the result of collision
                self.state = obs = next_state
                self.traj.append(tuple(next_state[[0,1,3]])) # using tuple it will be appended by value
            ...
            if not (self.mode=="selfplay" and len(a)>2 and a[-1]==1): # to avoid finishing when Alice plays
                reward, done = self.compute_reward(next_state, self.goal, None)
            done |= (len(self.traj)>=MAX_TRAJ_LEN and not(len(a)>2 and a[-1]==0)) # don't want max traj for Bob
    ...
    # It doesn't yet work if the random reset is used
    def _set_reachable_target(self):
        """ Perform a number of random steps in self and set 
            the final state to target that is now for sure reachable.
            Requires reset afterwards
        """
        assert self.traj==[]
        old_mode = self.mode
        self.mode = "selfplay"

        # Generate a reachable target by a random walk
        t_len = self.np_random.randint(2,MAX_STEPS+1) # randomly long gear chain [2, MAX_STEPS]
        while len(self.traj) < t_len and self.steps<10: # to avoid trying for too long
            a_idx = self.np_random.randint(self.action_space.n)
            phi = ANGLES[a_idx//len(DIAMS)]
            d = DIAMS[a_idx%len(DIAMS)]
            self.step([d, phi,-1]) # pretend Alice is playing to avoid finishing after first step

        self.set_target(self.state[:2], self.state[2])
        self.mode = old_mode
        self.state = np.array(self.state_init)
        self.traj = []
        self.steps = 0 
        self._init_fig()
...
if __name__=="__main__":
    ...
    flag = True
    while flag:
        d, phi = input("Action (d,phi) or q to quit:").split() # 0<d<1, 0<phi<2pi

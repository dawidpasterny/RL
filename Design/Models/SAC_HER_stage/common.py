...
class Agent_SAC_HER():
    def __init__(self, act_net, env, buffer, gamma, device="cpu", **kwargs):
        ...
        self.clip = lambda x: [min(max(0.04,x[0]), .9), min(max(0,x[1]), 1)] 
        # self.clip = lambda x: [min(max(.04,x[0]), .9), x[1]%1] 
    ...
    def play_episode(self, random=True):
        """ Play an entire episode (because of HER and since they won't get much longer either).
            ...
            - random: wheater new map will be created after each attempt or not

            Returns the number of newly added experiences (i.e. steps)
        """
        # encoded pixel state concatenated with current state and target
        obs = self.env.reset(random=random)["observation"]
        ...
        while not done:
            ...
            if info.get("invalid", False):
                continue # to skip adding failed (i.e. exactly same) obs to bufer
            
            next_obs = np.hstack((next_obs["observation"], target))
            total_reward += reward
            # print("Next state: ", next_state)

            local_buffer.append([obs, action, reward, done, next_obs])
...
class ExperienceBuffer:
    ...
    def sample(self, batch_size):
        ...
        obs_t = torch.FloatTensor(np.array(obs, dtype=np.float32)).to(self.device)
        actions_t = torch.FloatTensor(np.array(actions, dtype=np.float32)).to(self.device)
        rewards_t = torch.FloatTensor(np.array(rewards, dtype=np.float32)).to(self.device)
        next_obs_t = torch.FloatTensor(np.array(next_obs, dtype=np.float32)).to(self.device)
        dones_t = torch.BoolTensor(dones).to(self.device)
        # print(torch.count_nonzero(dones_t))

        return obs_t, actions_t, rewards_t, dones_t, next_obs_t

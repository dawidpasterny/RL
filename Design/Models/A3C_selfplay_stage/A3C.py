...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir=PATH+"A3C_selfplay_stage//runs/"+datetime.datetime.now().strftime("%b%d_%H_%M_%S"))
    ...
    alice = AC(NUM_FEATURES+7, 2, fe).share_memory().float() # lazy state size 
    bob = AC(NUM_FEATURES+7, 2, fe).share_memory().float()
            ...
            while True:
                ...
                if len(train_entry)>2:
                    ...
                    if best_mean_reward is None or best_mean_reward < mean_r_b:
                        ...
                        torch.save(alice.state_dict(), PATH+f"A3C_selfplay_stage/Alice-{args.job}.dat")
                        torch.save(bob.state_dict(), PATH+f"A3C_selfplay_stage/Bob-{args.job}.dat") 

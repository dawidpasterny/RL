...
class Agent_SAC_HER():
    ...
    def play_episode(self, random=True):
        ...
        t_len = self.env.np_random.randint(2,5) # randomly long gear chain
        while len(self.env.traj) < t_len and self.env.steps<10:
            a_idx = self.env.np_random.randint(self.env.action_space.n)
            phi = sc.ANGLES[a_idx//len(sc.DIAMS)]
            d = sc.DIAMS[a_idx%len(sc.DIAMS)]
            self.env.step([d, phi,-1]) # pretend Alice is playing to avoid finishing after first step

        self.env.set_target(self.env.state[:2], self.env.state[2])
        ...
        while not done:
            ...
            action_clipped = self.clip(action) # [d, phi]
